...
function showAllDataTM24h(): string {
    ...
    if(!empty($data)) {
        ...
        show_message("Pas de données disponibles / <em>No data vailable</em>.", false);
        return "";
    }
}
...
if (($_SERVER['REQUEST_METHOD'] === 'POST') && !empty($_POST)) {
    ...
    if (!isset($callsign) || $callsign=="") {
        show_message("OM non sélectionné. / <em>OM not selected.</em> ?", true);
    }
    else {
        filter_form($callsign);
    }
    if (!isset($action) || $action=="") {
        show_message("Quelle action souhaitez-vous faire SVP / <em>What action are you intending to execute</em> ?", true);
    }
    else {
        filter_form($action);
    }
    if($action=="Activation") {
        ...
        if (!isset($mode) || $mode=="") {
            show_message("Mode non renseigné / <em>Mode not specified</em>.", true);
        }
        else {
            filter_form($mode);
        }
        if (!isset($frequence)) {
            show_message("Frequence non renseignée / <em>Frequency not specified</em>.", true);
        }
        else {
            ...
            if($frequence>=$frequencemin && $frequence<=$frequencemax) {
                ...
                show_message("Fréquence en dehors de la plage autorisée dans le TNRBF<br><en>Frequency outside of the allowable range specified in the NFAT</em>.", true);
            }
        }
        if (!isset($status) || $status=="") {
            show_message("Status non renseigné / <em>Status not specified</em>.", true);
        }
        else {
            filter_form($status);
        }
