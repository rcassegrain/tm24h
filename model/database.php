...
class DataBaseTM24h {
    ...
    public static function setPDO() :pdo {
        ...
        try {    
            ...
            show_message( "Erreur de connexion à la base de données / <em>Database connexion error </em> : " . $e->getMessage(), false);
        }
        return $pdo;
    }
...
class OMTM24h {
    ...
    public function CheckOMBD(): bool {
        ...
        if(empty($user)) {
            show_message("OM non identifié / <em>OM not identified</em>.", false);
            return false;
        } else {
            $stmtpwd->execute();
            ...
            if(password_verify( $password,$pwd[0])) {
                show_message("OM identidié / <em>OM identified</em>.", false);
                return true;
            } else {
                show_message("Mot de passe incorrect, veuillez réessayer / <em>Incorrect password, please try again</em>.", false);
                return false;
            }
        }
    }
    ...
    public function SetDataTM24h() {
        ...
        if(empty($result)) {
            ...
            if($status=="Active") {
                ...
                show_message("Activation non effectuée au prélable, désactivation impossible<br><em>Activation not performed previously, Desactivation not possible</em>.", false);
            }
        // Activation exists already in the database.
        } else {
            if($status=="Active") {
                show_message("Activation déjà effectuée / <em>Activation already performed</em>.", false);
            } else {
                $pdo = DataBaseTM24h::setPDO();
